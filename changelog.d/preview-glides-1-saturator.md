...
settled stage's own arithmetic, and inside the period the four curve coefficients (k, bias, tanh(k·bias), the peak
normaliser; oversampled rate) and drive-compensation, mix and trim (base rate) are interpolated per sample between the
two designs — as `start + d·index`, product and sum in separate statements (law 10), so a coefficient is a function
of the sample's place in its period and never of a call. Measured the same way: -63.8, -67.2, -63.6, -67.3, -65.2,
...
that. A write that changes nothing continuous costs a comparison (it used to cost a full re-design), so a caller that
re-sends its parameters every block pays nothing. The ramp accumulates in double, landing on the target itself. New
