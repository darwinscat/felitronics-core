...
#
# Produce the CSVs (one per machine), then render:
#   FCORE_SWEEP_ONLY=1 ./build-juce/tools/fcore_fftbench | grep -E '^  [0-9]+[*]?,' > i9.csv   # on the i9
#   FCORE_SWEEP_ONLY=1 ./build-juce/tools/fcore_fftbench | grep -E '^  [0-9]+[*]?,' > m5.csv   # on the M5
#   python3 tools/plot-convolver-sweep.py i9.csv m5.csv docs/assets/nupc-vs-juce.svg
# CSV columns: buffer[*],ms,juce_pct,nupc_pct,nupc_maxpct. On a hybrid CPU pin the bench to a performance core.
import math, re, sys
...
def load(p):
    ...
    for line in open(p):
        ...
        B.append(b); j.append(float(parts[2])); u.append(float(parts[3])); m.append(float(parts[4]))
    return B, j, u, m
...
def cross_idx(j, u):
    n = len(j)
    for i in range(n - 1):
        if j[i] < u[i] and j[i + 1] < u[i + 1]: return i
    return n - 1 if j[-1] < u[-1] else n
...
def panel(x0, y0, cpu, env, juce, us, usMax):
    ...
    s = []
    usMean = sum(us) / len(us); jMin = min(juce)
    ...
    cap = (f'NUPC flat mean {usMean:.2f}%  ·  JUCE {jMin:.2f}–{juce[0]:.1f}%  ·  '
    ...
    s.append(f'<text x="{x1+pw/2:.1f}" y="{y1+ph+38:.1f}" font-family="{MONO}" font-size="11" fill="{INK2}" text-anchor="middle">host buffer size (samples, log scale · measured every 32)</text>')
    ...
    def path(a): return "M" + " L".join(f"{X(i):.1f} {Y(a[i]):.1f}" for i in range(len(a)))
    s.append(f'<path d="{path(juce)}" fill="none" stroke="{JUCE}" stroke-width="2" stroke-linejoin="round"/>')
    s.append(f'<path d="{path(us)}" fill="none" stroke="{US}" stroke-width="2" stroke-linejoin="round"/>')
    li = len(B) - 1
    ...
    s.append(f'<text x="{X(0)+6:.1f}" y="{Y(juce[0])-7:.1f}" font-family="{MONO}" font-size="12" font-weight="600" fill="{JUCE}">{juce[0]:.0f}%</text>')
    return "\n".join(s)
...
out = [f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}" font-family="{SANS}">',
       ...
       f'<text x="28" y="55" font-family="{MONO}" font-size="11.5" fill="{INK3}">MatrixConvolverNupc vs juce::dsp::Convolution 8.0.4 · 131072-tap stereo (LRDiag) · 48 kHz · mean %RT = CPU ÷ audio time</text>']
lx = 28
for col, txt, band in [(US, "NUPC mean", False), (US, "NUPC worst buffer (band)", True), (JUCE, "juce::dsp::Convolution", False), (DANGER, "100% real-time", False)]:
