...
static void testChannelGateSilence()
{
    ...
    for (int mode = 0; mode < 2; ++mode)   // 0: 2->1->2   1: 2->3->2 with four channels prepared
    {
        ...
        b.processBlock (ch, 2, N);                                    // one block so the design is applied
...
static void testPreventionOnSurvivingChannel()
{
    ...
    for (int mode = 0; mode < 2; ++mode)
    {
        ...
        for (int k = 0; k < charge + gap + tail; ++k)
        {
            ...
            dut.processBlock (dch, inGap ? gapNc : 2, N);
            ref.processBlock (rch, 2, N);                  // the reference never changes its channel count
...
static void testRecoveryCoefficients()
{
    ...
    for (int si = 0; si < 4; ++si)
    {
        ...
        for (int k = 0; k < 20; ++k) { dut.processBlock (dch, 2, N); ref.processBlock (rch, 2, N); }

        p.lane (Lane::Side).freq = 4000.0;                 // the edit, issued one block before the gap
        dut.setParams (p); ref.setParams (p);
        dut.processBlock (dch, 2, N); ref.processBlock (rch, 2, N);

        for (int k = 0; k < stretches[si]; ++k) { dut.processBlock (dch, 1, N); ref.processBlock (rch, 2, N); }
        dut.processBlock (dch, 2, N); ref.processBlock (rch, 2, N);
...
static void testRecoveryStrongNull()
{
    ...
    for (int k = 0; k < 40; ++k)                            // the gap: DUT parks the lane, REF hears silence
    {
        ...
        dut.processBlock (dch, 1, N);
        ref.processBlock (rch, 2, N);
    }
...
static void testNonChannelGates()
{
    ...
    //     to sit out the excursion and resume: measured 0.690 (-3.2 dBFS) out of silence at 100 Hz / Q10.
    {
        ...
        double ph = 0.0, charged = 0.0;
        for (int k = 0; k < 40; ++k) { fillSine (a, 100.0, 1.0, ph); c = a; b.processBlock (ch, 2, N); charged = peakOf (a); }
        ...
        for (int k = 0; k < 40; ++k) { std::fill (a.begin(), a.end(), 0.0f); c = a; b.processBlock (ch, 2, N); }
    ...
    //     DELTA bell was not, and that is the half this closes.
    {
        ...
        for (int k = 0; k < 40; ++k)
        {
            ...
            b.processBlock (ch, 2, N); charged = peakOf (a);
        }
        ok (charged > 0.1, "precondition: the Side delta bell really carried signal");

        p.lane (Lane::Side).on = false; b.setParams (p);
        for (int k = 0; k < 4; ++k) { std::fill (a.begin(), a.end(), 0.0f); c = a; b.processBlock (ch, 2, N); }
...
static void testWhatSurvives()
{
    ...
    //     graph with an empty block at a different width would silently wipe the stream it is about to run.
    {
        ...
        for (int k = 0; k < 16; ++k)
        {
            ...
            r0 = d0; r1 = d1;
            if (k == 8) dut.processBlock (dch, 1, 0);        // the empty probe, at a narrower width
...
int main()
{
    ...
    group ("RT-safety");
    {
        ...
        b.processBlock (ch, 2, 512);                          // warm: designs, applies, settles
        const int before = g_allocs.load();
        for (int k = 0; k < 40; ++k) b.processBlock (ch, (k % 3 == 0) ? 1 : ((k % 3 == 1) ? 3 : 2), 512);
