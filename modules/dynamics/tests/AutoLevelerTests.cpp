...
// JUCE-free self-tests for felitronics::dynamics::AutoLeveler — the wet->dry match is pure-numeric,
...
namespace
{
    // juce::Decibels::gainToDecibels (g, -120.0f), replicated exactly — the ported tests measure
    // the SAME quantity the original JUCE-based tests measured.
    float gainDb (float gain)
    {
        return gain > 0.0f ? std::max (-120.0f, std::log10 (gain) * 20.0f) : -120.0f;
    }
              ...
              float startGain = 1.0f)
    {
        ...
        for (int i = 0; i < blocks; ++i)
        {
            ...
            t.db.push_back (gainDb (a.currentGain()));
        }
        return t;
    }
    ...
    {
        const int i = std::clamp ((int) std::llround (sec * t.sr / t.blk) - 1,
                                  0, (int) t.db.size() - 1);
...
int main()
{
    std::printf ("felitronics::dynamics AutoLeveler tests\n");

    test::group ("converges to sqrt(dryMS / mixMS)");
    test::approx (settle (1.0, 0.25, true, 3000), 2.0f, 0.05f, "settled gain != sqrt(4)=2");

    test::group ("clamps to the +36 dB ceiling");
    test::approx (settle (1.0, 1.0e-9, true, 5000), 63.10f, 1.0f, "did not clamp at +36 dB");

    test::group ("clamps to the -24 dB floor");
    test::approx (settle (1.0, 1.0e6, true, 5000), 0.0631f, 0.01f, "did not clamp at -24 dB");

    test::group ("silence-gated: sub-floor dry energy holds unity");
    test::approx (settle (1.0e-8, 1.0e-12, true, 3000), 1.0f, 0.01f, "silence gate chased the noise floor");

    test::group ("disabled aims for unity even with a loud mismatch");
    test::approx (settle (1.0, 0.25, false, 3000), 1.0f, 0.02f, "disabled leveler moved off unity");
    ...
    test::group ("ANTI-FREEZE regression: 64-sample blocks converge at the designed rate");
    {
        ...
        test::approx (atTime (t, 2.2), -12.7f, 0.35f,
            "64-sample blocks: leveler frozen (the SmoothedValue double-limiter bug)");
    }

    test::group ("block-size invariance of the gain trajectory");
    {
        ...
        for (int blk : { 16, 64, 173, 2048 })
        {
            ...
            for (double sec : { 0.5, 1.0, 1.5, 2.2 })
                test::approx (atTime (t, sec), atTime (ref, sec), 0.25f,
                    "blk=" + std::to_string (blk) + " diverges from blk=512 @ " + std::to_string (sec) + " s");
        }
    }

    test::group ("sample-rate invariance of the gain trajectory (44.1/96/192 kHz)");
    {
        const auto ref = run (1.0, mixFor127, 512, 48000.0, 2.5);
        for (double sr : { 44100.0, 96000.0, 192000.0 })
        {
            ...
            for (double sec : { 0.5, 1.0, 2.2 })
                test::approx (atTime (t, sec), atTime (ref, sec), 0.30f,
                    "sr=" + std::to_string (sr) + " diverges @ " + std::to_string (sec) + " s");
        }
    }

    test::group ("hard 9 dB/s ceiling, sample by sample, under a pathological target");
    {
        ...
        float prevDb = gainDb (a.currentGain());
        ...
        for (int i = 0; i < blocks; ++i)
        {
            ...
            for (int s = 0; s < 64; ++s)
            {
                const float db = gainDb (a.getNextGain());
                slopeOk = slopeOk && (std::fabs (db - prevDb) <= maxStep);
                prevDb = db;
            }
            if (i >= blocks - 1500)   // the last 2 s: settled
                { lateMin = std::min (lateMin, prevDb); lateMax = std::max (lateMax, prevDb); }
        }
        test::ok (slopeOk, "per-sample slope exceeded 9 dB/s");
        const float meanRatioDb = 10.0f * std::log10 (1.0 / ((100.0f + 0.01f) * 0.5f));
        test::approx (prevDb, meanRatioDb, 0.6f,
            "did not converge to the mean-energy ratio");
        test::ok (lateMax - lateMin < 0.25f, "settled gain wobbles "
                                             + std::to_string (lateMax - lateMin) + " dB at the block rate");
    }

    test::group ("snapRatioTo: fast bounded glide that lands and STICKS");
    {
        AutoLeveler a;
        a.prepare (48000.0);
        // converge at unity ratio first
        for (int i = 0; i < 2000; ++i) { a.processBlock (1.0, 1.0, true, 64); for (int s = 0; s < 64; ++s) a.getNextGain(); }
        test::approx (a.currentGainDb(), 0.0f, 0.1f, "did not converge at unity before the snap");
        ...
        a.snapRatioTo (std::pow (10.0f, -12.0f * 0.05f));   // juce::Decibels::decibelsToGain (-12.0f)
        ...
        bool slopeOk = true, monotonicOk = true;
        for (int i = 0; i < 1500; ++i)   // 2 s
        {
            ...
            for (int s = 0; s < 64; ++s)
            {
                ...
                slopeOk     = slopeOk     && (std::fabs (db - prevDb) <= fastStep);
                monotonicOk = monotonicOk && (db <= prevDb + 3.0e-6f);
                prevDb = db;
            }
            if (landedAt < 0 && std::fabs (a.currentGainDb() + 12.0f) < 0.05f)
                landedAt = i;
        }
        test::ok (slopeOk, "snap glide exceeded 40 dB/s");
        test::ok (monotonicOk, "snap glide must be monotonic here");
        test::ok (landedAt >= 0 && landedAt * 64 / 48000.0 < 0.35,
                "snap did not land within the fast window");
        test::approx (a.currentGainDb(), -12.0f, 0.15f,
                "followers pulled the snapped makeup back toward the old spectrum");
    }

    test::group ("snap during silence holds, then sticks when signal returns");
    {
        AutoLeveler a;
        a.prepare (48000.0);
        for (int i = 0; i < 2000; ++i) { a.processBlock (1.0, 1.0, true, 64); for (int s = 0; s < 64; ++s) a.getNextGain(); }
        a.snapRatioTo (std::pow (10.0f, -6.0f * 0.05f));    // juce::Decibels::decibelsToGain (-6.0f)
        for (int i = 0; i < 500; ++i)  { a.processBlock (1.0e-9, 1.0e-9, true, 64); for (int s = 0; s < 64; ++s) a.getNextGain(); }   // silence
        test::approx (a.currentGainDb(), -6.0f, 0.15f, "silent snap didn't glide to target");
        ...
        test::approx (a.currentGainDb(), -6.0f, 0.25f, "makeup drifted after signal returned");
        test::ok (std::isfinite (a.currentGain()), "non-finite gain after silence");
    }

    test::group ("enable/disable toggles glide fast (deterministic retarget)");
    {
        AutoLeveler a;
        a.prepare (48000.0);
        const double mixFor12 = std::pow (10.0, 12.0 / 10.0);
        for (int i = 0; i < 4000; ++i) { a.processBlock (1.0, mixFor12, true, 64); for (int s = 0; s < 64; ++s) a.getNextGain(); }
        test::approx (a.currentGainDb(), -12.0f, 0.2f, "did not converge to -12 dB before the toggle");
        ...
        test::ok (n * 64 / 48000.0 < 0.45, "disable took " + std::to_string (n * 64 / 48000.0) + " s to reach unity");
        ...
        test::ok (n * 64 / 48000.0 < 0.45, "re-enable took " + std::to_string (n * 64 / 48000.0) + " s to return");
    }
