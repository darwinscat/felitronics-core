...
//==============================================================================
// felitronics::dynamics::AutoLeveler — the wet->dry loudness match. Two slow one-pole RMS
...
// (Extracted from OrbitCab, where it levels the convolved cab against the dry guitar.)
//
...
// gain — no buffers, no framework types — so it's trivially unit-testable and portable.
//
// 🔴 RT-safe: every method is allocation-, lock-, IO- and exception-free — plain arithmetic
// on value members. prepare()/reset()/seed() are for stream (re)starts; the rest may run on
// the audio thread.
//==============================================================================
...
public:
    ...
    // while the followers crawl up from zero (OrbitCab #48). The followers are set to a
    // consistent (dry, mix) pair so the slow follower doesn't yank the makeup during
    // convolver warm-up.
    void seed (float makeupGain) noexcept
    {
        const float g = std::clamp (makeupGain, kMatchMinGain, kMatchMaxGain);
        targetDb  = gainToDecibels (g, kSilentDb);
        targetGain = g;
        appliedGain = g;
        fastSamplesLeft = 0;
    ...
    void snapRatioTo (float makeupGain) noexcept
    {
        const float g = std::clamp (makeupGain, kMatchMinGain, kMatchMaxGain);
        targetDb = gainToDecibels (g, kSilentDb);
        targetGain = g;
    ...
    void processBlock (double dryBlockMeanSq, double mixBlockMeanSq, bool enabled, int numSamples) noexcept
    {
        ...
        if (! enabled)
            ...
            rawTarget = std::clamp ((float) std::sqrt (dryMeanSq / (mixMeanSq + 1.0e-12)),
                                    kMatchMinGain, kMatchMaxGain);
        else
        ...
        const float rawDb = gainToDecibels (rawTarget, kSilentDb);
        if (enabledFlipped)
        {
        ...
        targetDb = std::clamp (rawDb, targetDb - stepDb, targetDb + stepDb);
        targetGain = decibelsToGain (targetDb, kSilentDb);   // one pow per block
    }
    ...
    float currentGain()   const noexcept { return appliedGain; }
    float currentGainDb() const noexcept { return gainToDecibels (appliedGain, kSilentDb); }
    ...
    bool snapGliding() const noexcept { return fastSamplesLeft > 0; }
    ...
    void openTransitionWindow() noexcept
    {
        fastSamplesLeft = (int) std::lround (kSnapWindowSeconds * currentSampleRate);
    }

private:
    // juce::Decibels::{gainToDecibels, decibelsToGain} replicated EXACTLY (incl. the explicit
    // minusInfinityDb floor semantics and float-precision log10/pow), so the extraction is
    // numerically identical to the JUCE-based original:
    //   gainToDecibels: gain > 0 ? max (floor, log10 (gain) * 20) : floor
    //   decibelsToGain: db > floor ? 10^(db * 0.05) : 0
    static float gainToDecibels (float gain, float minusInfinityDb) noexcept
    {
        return gain > 0.0f ? std::max (minusInfinityDb, std::log10 (gain) * 20.0f)
                           : minusInfinityDb;
    }

    static float decibelsToGain (float decibels, float minusInfinityDb) noexcept
    {
